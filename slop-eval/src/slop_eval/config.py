"""Runtime configuration for the empathetic evaluation layer.

By default slop-eval resolves operations **offline**, against a local engine,
...
class Settings:
    ...
    api_key: str | None = os.environ.get("SLOP_EVAL_API_KEY")

    # Endpoint for the local Ollama backend.
    ollama_url: str = os.environ.get(
        "SLOP_EVAL_OLLAMA_URL", "http://localhost:11434/api/generate"
    )

    # When False (default), operations are resolved by the offline local engine.
    # Set live=True (or SLOP_EVAL_LIVE=1) to route through the configured
    # backend. Any backend failure degrades cleanly back to offline resolution.
    live: bool = _env_flag("SLOP_EVAL_LIVE", False)

    # Token budget per resolved evaluation.
    max_tokens: int = int(os.environ.get("SLOP_EVAL_MAX_TOKENS", "1024"))
...
def configure(
    *,
    ...
    Example::
        ...
        se.configure(provider="openai", live=True)
    """
