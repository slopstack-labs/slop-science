"""Runtime configuration for the executive communication resolution layer.

By default slop-report resolves operations **offline**, against a local engine,
...
class Settings:
    ...
    api_key: str | None = os.environ.get("SLOP_REPORT_API_KEY")

    # Endpoint for the local Ollama backend.
    ollama_url: str = os.environ.get(
        "SLOP_REPORT_OLLAMA_URL", "http://localhost:11434/api/generate"
    )

    # When False (default), operations are resolved by the offline local engine.
    # Set live=True (or SLOP_REPORT_LIVE=1) to route through the configured
    # backend. Any backend failure degrades cleanly back to offline resolution.
    live: bool = _env_flag("SLOP_REPORT_LIVE", False)

    # Token budget per resolved report section.
    max_tokens: int = int(os.environ.get("SLOP_REPORT_MAX_TOKENS", "1024"))
...
def configure(
    *,
    ...
    Example::
        ...
        sr.configure(provider="openai", live=True)
    """
