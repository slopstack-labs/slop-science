"""Runtime configuration for the inference layer.

By default slop-frames resolves operations **offline**, against a local engine,
so the package is fully functional without network access or credentials. Enable
live resolution with ``live=True`` and select a backend with ``provider=...``.
"""

from __future__ import annotations

import os
from dataclasses import dataclass


def _env_flag(name: str, default: bool = False) -> bool:
    raw = os.environ.get(name)
    if raw is None:
        return default
    return raw.strip().lower() in {"1", "true", "yes", "on"}


@dataclass
class Settings:
    """Global configuration for tabular resolution."""

    # Inference backend: "anthropic" (default), "openai", "google", or "ollama".
    provider: str = os.environ.get("SLOP_FRAMES_PROVIDER", "anthropic")

    # Model identifier. Empty selects the provider default (e.g. claude-opus-4-8
    # for Anthropic, gpt-4o for OpenAI).
    model: str = os.environ.get("SLOP_FRAMES_MODEL", "")

    # Credential for the selected backend. Falls back to the backend's native
    # environment variable (ANTHROPIC_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY).
    api_key: str | None = os.environ.get("SLOP_FRAMES_API_KEY")

    # Endpoint for the local Ollama backend.
    ollama_url: str = os.environ.get(
        "SLOP_FRAMES_OLLAMA_URL", "http://localhost:11434/api/generate"
    )

    # When False (default), operations are resolved by the offline local engine.
    # Set live=True (or SLOP_FRAMES_LIVE=1) to route through the configured
    # backend. Any backend failure degrades cleanly back to offline resolution.
    live: bool = _env_flag("SLOP_FRAMES_LIVE", False)

    # Token budget per resolved cell.
    max_tokens: int = int(os.environ.get("SLOP_FRAMES_MAX_TOKENS", "1024"))

    # Controls the failure mode of an over-constrained join. When safe_mode is
    # True, an aggressive `slop_merge` reports the failure but suppresses the
    # terminating SIGSEGV. Recommended for notebooks and CI.
    safe_mode: bool = _env_flag("SLOP_FRAMES_SAFE_MODE", False)


settings = Settings()


def configure(
    *,
    provider: str | None = None,
    model: str | None = None,
    api_key: str | None = None,
    ollama_url: str | None = None,
    live: bool | None = None,
    max_tokens: int | None = None,
    safe_mode: bool | None = None,
) -> Settings:
    """Update the global configuration in place and return it.

    Example::

        import slop_frames as sf
        sf.configure(provider="openai", live=True)
    """
    if provider is not None:
        settings.provider = provider
    if model is not None:
        settings.model = model
    if api_key is not None:
        settings.api_key = api_key
    if ollama_url is not None:
        settings.ollama_url = ollama_url
    if live is not None:
        settings.live = live
    if max_tokens is not None:
        settings.max_tokens = max_tokens
    if safe_mode is not None:
        settings.safe_mode = safe_mode
    return settings
